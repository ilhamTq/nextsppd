...
const DeletePejabat = ({ pejabatt }) => {
  ...
  const handleDelete = async () => {
    console.log('hapus',pejabatt.id)
    const result = await fetch(`/api/pjsppd/${pejabatt.id}`,{method:"DELETE"})
    setIsOpen(false);
    router.refresh();
  };
    ...
    <div>
      ...
      <div className={isOpen ? "modal modal-open" : "modal"}>
        <div className="modal-box my-0 mb-8">
          <h3 className="font-bold text-lg">Hapus Pejabat {pejabatt.nama}?</h3>
