...
const DeleteGolongan = ({golongan}) => {
  ...
  const handleDelete = async () => {
    console.log('hapus',golongan.id)
    const result = await fetch(`/api/golongan/${golongan.id}`,{method:"DELETE"})
    setIsOpen(false);
    router.refresh();
  };
    ...
    <div>
      ...
      <div className={isOpen ? "modal modal-open" : "modal"}>
        <div className="modal-box my-0 mb-8">
          <h3 className="font-bold text-lg">Hapus Golongan?</h3>
